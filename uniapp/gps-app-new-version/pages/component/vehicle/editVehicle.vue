                ...
                uni.request({
                    ...
                    success: (res) => {
                        ...
                        if (res.data.error_code === '000000') {
                            ...
                            if (typeof(res.data) === 'string' && res.data.includes('error_code')) {
                                let data = dataAxios.data.split('==>')[0];
                                let errorData = JSON.parse(data.concat('"}'));
                                this.toastRequestErr(errorData['error_code'], errorData['error_desc'])
                            } else {
                                this.toastRequestErr(res.data.error_code, res.data.error_desc);
                            }
                        }
                    },
                    fail: () => {
